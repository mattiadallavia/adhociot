...
\begin{enumerate}
\item $n$ nodi satellite (i sensori), ciascuno identificato da un indice $i \in \{1, \dots, n\}$ univoco.
\item Un nodo centrale detto \emph{sink}, di indice $i = 0$.
\end{enumerate}

Tutti i nodi hanno la stessa portata (del segnale) $p$. Il nodo \emph{sink} costituisce il destinatario finale di tutti i messaggi. Questo nodo svolge il ruolo di \emph{gateway} verso il sistema informativo sovrastante.

Chiameremo \emph{ambiente} un disco (regione di spazio) di raggio $a$ centrato nel \emph{sink}, nel quale sono distribuiti i satelliti.
...
Dato un \emph{grafo realistico} $G$, esistono in realtà infiniti \emph{layout equivalenti} tali che $G = graph(L_i)$ che vengono mappati sullo stesso grafo. In altre parole $graph(L)$ non è iniettiva. In particolare, dato
\begin{equation*}
...
\end{equation*}
con $m \in \mathbb{Q}_{>0}$. I \emph{layout in scala} sono tutti \emph{equivalenti} poiché
\begin{equation*}
...
\end{align*}
questi layout sono \emph{equivalenti}, in particolare $L_{15} = scale(L_5,\,3)$. Entrambi vengono quindi mappati nello stesso grafo
\begin{equation*}
...
Alla luce di quanto appreso nella sezione precedente, introduciamo ora il concetto di \emph{dimensione del layout}. Questo esprime il raggio ambiente in unità di portate, ovvero quanto è più grande $a$ rispetto a $p$. In altre parole è il raggio ambiente normalizzato.
