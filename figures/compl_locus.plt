...
fit l(x,w_c,t_c) 'stat.dat' \
    using ((($4 >= 0.45) && ($4 <= 0.55)) ? $1 : NaN):2:($4 - 0.5) via w_c,t_c

fit l(x,w_t,t_t) 'stat.dat' \
    using ((($4 >= 0.85) && ($4 <= 0.95)) ? $1 : NaN):2:($4 - 0.9) via w_t,t_t
...
plot l(x,w_c,t_c) with filledcurves y=1000 palette cb 0.5 notitle fillstyle pattern 6, \
	 ...
	 'stat.dat' using ((($4 >= 0.45) && ($4 <= 0.55)) ? $1 : NaN):2:4 palette pointtype 3 notitle, \
	 'stat.dat' using ((($4 >= 0.85) && ($4 <= 0.95)) ? $1 : NaN):2:4 palette pointtype 3 notitle, \
