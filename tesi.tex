...
Tra i nodi adiacenti, ossia alla stessa profondità. In questo caso l'idea è quella di selezionare i canali in modo che ciascun nodo di una data profondità trasmetta su un canale distinto. Questo è possibile grazie alle informazioni presenti nei messaggi di riscontro ed è perciò implementato nella routine di ricezione.
...
Se $state(i) = waiting$, il nodo viene attivato ($state(i) = active$), abilitando quindi $i$ a trasmettere. I satelliti attivati dallo stesso messaggio di esplorazione corrono il rischio di effettuare la prima trasmissione nello stesso istante e canale, dando luogo ad una collisione. Per mitigare questo fenomeno scheduliamo la prima trasmissione mediante un criterio euristico basato sulla distanza da $j$ (stimata mediante la potenza ricevuta):
\begin{equation*}
wait(i) = frame(t) + w \cdot dist(i,\,j)
\end{equation*}
dove $w \in \mathbb{Q}_{>0}$ è il \emph{fattore di attesa}. La speranza è che l'attesa fornisca una diversificazione degli invii sufficiente a evitare interferenze. $w$ rappresenta il compromesso tra probabilità di collisione e rapidità di raccolta dei dati, va perciò ricavato in funzione delle caratteristiche della rete.

La profondità viene ricalcolata come $depth(i) = min(depth(i),\ depth(j)+1)$. Potrebbe infatti esistere un percorso più breve al sink di quanto stimato in precendenza.
...
Se $depth(j) < depth(i)$, $node\_confirm(m) \neq i$ e $channel\_confirm(m) = channel(i)$, significa che un nodo adiacente (di profondità $depth(i)$) ha consegnato con successo un messaggio sul canale $channel(i)$. È perciò necessario selezionare un nuovo canale: $channel(i)$\textit{++}.

Se $node\_confirm(m) = i$, stiamo ricevendo il riscontro di una precedente trasmissione di $m$. Va quindi azzerato il contatore dei tentativi di trasmissione $attempts(i) = 0$ e rischedulate le trasmissioni al prossimo quanto temporale disponibile $wait(i) = 0$.
...
node 0 transmits message 0 on channel 0
node 1 activated at depth 1, transmission scheduled for frame 7
node 4 activated at depth 1, transmission scheduled for frame 8
...
node 1 transmits message 1 on channel 0
added by node 0
node 2 activated at depth 2, transmission scheduled for frame 17
node 3 activated at depth 2, transmission scheduled for frame 15
node 1 scheduled attempt n. 2 at frame 9
...
node 0 transmits message 1 on channel 0
node 1 received an ack, transmission scheduled for frame 8
message 1 removed from node 1
channel 0 in use by node 1, node 4 selects channel 1
...
node 4 transmits message 4 on channel 1
added by node 0
node 4 scheduled attempt n. 2 at frame 9
...
node 0 transmits message 4 on channel 0
node 4 received an ack, transmission scheduled for frame 9
message 4 removed from node 4

t=47 (frame 15, slot 2)
...
node 3 transmits message 3 on channel 0
added by node 1
node 3 scheduled attempt n. 2 at frame 17

t=49 (frame 16, slot 1)
...
\subsection{layout.c}
\lstinputlisting{layout.c}

\subsection{graph.c}
\lstinputlisting{graph.c}

\subsection{conn.c}
\lstinputlisting{conn.c}
